...
{$R *.dfm} //Radio Group começa do ZERO
////////////////////////////////////////////////
procedure TExRadioButon.btnadicionarClick(Sender: TObject);//Adicionar itens

begin //saber se um Item já está dentro = vai dentro da lista ver se o item já existe
...
begin
    case rgOpcoes.ItemIndex of //Pode ser If ou Case
        -1 : ShowMessage ('Nenhum seleconado');
         0 : ShowMessage ('Opção 1 selecionado');
         2 : ShowMessage ('Opção 2 selecionado');
         3 : ShowMessage ('Opção 3 selecionado');
         4 : ShowMessage ('Opção 4 selecionado')
    else //Começa do zero, por isso coloca + 1
        ShowMessage ('Opção ' + IntToStr(rgOpcoes.ItemIndex + 1) + ' selecioado');
        //rgOpcoes.ItemIndex = tem que converter para string, porque é inteiro
    end;
...
begin
    if rbOpcao1.Checked then //Testa cada u para ver se está selecionado
       ShowMessage ('Botão 1 selecionado')
    else
       if rbOpcao2.Checked then
          ShowMessage ('Botão 2 selecionado');
